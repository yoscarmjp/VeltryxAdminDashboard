...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action == 'approve') {
        ...
        $upd->execute([$id]);
        log_action("APPROVE_APP", "Aprobó la postulación de $app_name (Staff)");
    } elseif ($action == 'reject') {
        $upd = $conn->prepare("UPDATE applications SET status='rejected' WHERE id=?");
        $upd->execute([$id]);
        log_action("REJECT_APP", "Rechazó la postulación de $app_name (Staff)");
    } elseif ($action == 'delete' && $_SESSION['role'] === 'owner') {
        $del = $conn->prepare("DELETE FROM applications WHERE id=?");
        $del->execute([$id]);
        log_action("DELETE_APP", "Eliminó la postulación de $app_name (Staff)");
    }
    header("Location: staff");
    exit();
}

$apps_stmt = $conn->query("SELECT * FROM applications WHERE job_type IN ('helper', 'builder', 'developer') ORDER BY created_at DESC");
...
<div class="header">
    <div>
        ...
        <p>Revisa y gestiona las solicitudes de los futuros miembros del equipo.</p>
    </div>
</div>
            ...
            <tr>
                ...
                <td style="text-align: right;">
                    <div class="actions" style="justify-content: flex-end; align-items: center; gap: 1rem;">
                        ...
                        <?php endif; ?>
                        <a href="app_details?id=<?php echo $app['id']; ?>&back=staff" class="btn-action view" style="background: rgba(59,130,246,0.15); color: var(--primary);" title="Ver Detalles"><i class="fa-solid fa-eye"></i></a>
